...
def main() -> None:
    ...
    if next_auto:
        stats_lines.append(f"  [bold]{next_auto}[/bold]")
    stats_lines.append(
        f"  [bold]Relevancia:[/bold]  "
    ...
    review_table.add_row(
        "Aceptados", make_bar(accepted, total, 20, "green"), str(accepted), f"{(accepted / total * 100):.0f}%"
    )
    review_table.add_row(
        "Rechazados", make_bar(rejected, total, 20, "red"), str(rejected), f"{(rejected / total * 100):.0f}%"
    )
    review_table.add_row(
        "Revisados", make_bar(reviewed, total, 20, "blue"), str(reviewed), f"{(reviewed / total * 100):.0f}%"
    )
    review_table.add_row(
        "Pendientes", make_bar(pending, total, 20, "yellow"), str(pending), f"{(pending / total * 100):.0f}%"
    )
    ...
    if pending_gaps:
        ...
        console.print("    [green]Todos los gaps cubiertos[/green]")
    ...
    if high_papers:
        console.print()
        console.print("  [bold]Ultimos papers de ALTA relevancia:[/bold]")
        for i, p in enumerate(high_papers[:5], 1):
            title_short = p.title[:55] + ("..." if len(p.title) > 55 else "")
            year_str = str(p.year) if p.year else "N/A"
            console.print(
                f"    {i}. [cyan]{title_short}[/cyan] ({year_str}) - Score: [yellow]{p.relevance_score}[/yellow]"
            )
    ...
    if source_counts:
        console.print()
        console.print("  [bold]Fuentes API:[/bold]  ", end="")
        parts = []
